...
public class Turret extends CommandRobot {
  ...
  private final ProfiledPIDController pid = new ProfiledPIDController(kP, kI, kD, new TrapezoidProfile.Constraints(0.5, 5));
  ...
  private Turret(TurretIO hardware) {
    ...
    sysIdRoutine = new SysIdRoutine(new SysIdRoutine.Config(QUASISTATIC_VOLTAGE, DYNAMIC_VOLTAGE), Turret turret, 5);
  }
  ...
  public void update(){
    hardware.setVoltage(pid.calculate(position(), MathUtil.clamp(goal, 0, 270)) + ff.calculateWithVelocities(velocity(), velocitySetpoint()));
  }

  public Command goTo(int goal) {
    return Commands.run(
        () -> {
          hardware.setVoltage(pid.calculate(position(), MathUtil.clamp(goal, 0, 270)) + ff.calculateWithVelocities(velocity(), velocitySetpoint()));
        });
